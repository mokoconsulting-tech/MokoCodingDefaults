...
require_once '../lib/sample.lib.php';
...
// Translations
$langs->loadLangs(array("admin", "sample@sample"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('samplesetup', 'globalsetup'));
...
$item = $formSetup->newItem('SAMPLE_MYPARAM1');
$item->fieldParams['isMandatory'] = 1;
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('SAMPLE_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';
$item->helpText = 'Tooltip text';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('SAMPLE_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('SAMPLE_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('SAMPLE_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('SAMPLE_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('SAMPLE_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('SAMPLE_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('SAMPLE_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('SAMPLE_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('SAMPLE_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('SAMPLE_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf SAMPLE_MYPARAM10
$item = $formSetup->newItem('SAMPLE_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('SAMPLE_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('SAMPLE_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('SAMPLE_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/sample/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=sample-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'SAMPLE_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'SAMPLE_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-sample page-admin');
...
$head = sampleAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "sample@sample");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("SampleSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'SAMPLE_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'SAMPLE_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
